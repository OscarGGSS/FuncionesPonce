...
void SubMenuVentas(TaVentas *, int *);
...
void RegistrarVenta(TaVentas *, int *);
...
//Funciones "externas"
int ValidarCliente(int);
int ValidarIdCliente();



void SubMenuVentas(TaVentas *aVentas, int *eTotalVentas) {
    ...
    do {
		...
		switch (eOpcion) {
		    ...
		    case 1: RegistrarVenta(aVentas, eTotalVentas);
                	getch();
                	break;
...
int CargarVentas(TaVentas aVentas, int *eTotalVentas) {
...
void RegistrarVenta(TaVentas *aVentas, int *eTotalVentas) {
    ...
    do {
		...
		rVenta.eIdCliente = ValidarIdCliente(); //valida que el Id sea un entero y que exista
		
		if (rVenta.eIdCliente == -1) { //ValidarIdCliente regresa -1 si el usuario ingresa 'x' o 'X'
   
   			break; //saltarse todo (regresa al menú)
...
void CapturarDatosVenta(TrVenta *pVenta) {
    ...
    Mensaje(9, 5, "Fecha de la venta:");
    rCopia.rFechaVenta = LeerFecha(10, 5);

    Mensaje(15, 5, "[1- MERCADO LIBRE     2-SHOPIFY     3-INSTAGRAM]");
    Mensaje(16, 5, "Plataforma:");
    
    eOpcion = LeerEnteroEnRango(16, 17, 1, 3);
...
int ValidarCliente(int eIdCliente) {
...
int ValidarIdCliente() {
    ...
    do {
    	...
    	BorrarZona(6, 28, 1, 30);
		BorrarZona(26, 20, 1, 80);

		sprintf(sMensaje, "Ingrese 'x' para regresar al men%c", 163);
    	Centrar(sMensaje, 26);
    
        Mensaje(6, 5, "Ingrese ID del cliente: ");
        GoToXY(6, 29);
        gets(sCadena);

        if (strcmp(sCadena, "x") == 0 || strcmp(sCadena, "X") == 0) {

            eIdCliente = -1;
            
            break;
            
        } else if (ValidarEntero(sCadena)) {
        	
            eIdCliente = atoi(sCadena);
            
            if (ValidarCliente(eIdCliente)) {
        	...
        	BorrarZona(26, 10, 1, 80);
            Error(4); //Error (El Id no es válido)
            getch();
